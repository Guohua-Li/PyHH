...
class Experiment:
  ...
  def run(self, t, dt):
    ...
    self.C_Clampers = []
    for cp in self.cmptList:
      if cp.cClamp: self.C_Clampers.append(cp.cClamp)
    ...
    for cp in self.cmptList:
      ...
      elif cp.cClamp and cp.cClamp not in self.C_Compartments:
        self.C_Compartments.append(cp)
      elif cp.iClamp and cp.iClamp not in self.I_Compartments:
        self.I_Compartments.append(cp)
      else:
        self.F_Compartments.append(cp)

    self.N_Compartments = self.I_Compartments + self.C_Compartments + self.F_Compartments
